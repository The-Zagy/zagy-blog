...
export default function LatestPosts() {
    ...
    const Posts = () => {
        ...
        if (posts.isSuccess) {
            return <div className="relative grid justify-center grid-cols-3 sm:grid-cols-6 gap-x-4 md:grid-cols-9 lg:grid-cols-12 lg:gap-x-6 mx-auto max-w-7xl py-3">{posts.data.map(post => <PostCard post={post} />)}</div>
        }
        return <></>
    }
    return (
